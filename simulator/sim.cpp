...
struct WaterParticle {
    WaterParticle(float t0, float spd, Color col, int pid)
        ...
        radial_r = static_cast<float>(rand()) / RAND_MAX * 0.8f; 
...
if (imbalance > total_demand * 0.1f) {
...
float getEffectivePressure() const {
    ...
    bool isStale() const {
        return (time(nullptr) - last_update_time) > 5; 
    }
...
Pipe(int id, Vec3 s, Vec3 e, PipeType t, float diam, int zoneID = -1)
    ...
    // ================== CONSISTENT DIAMETER DEFINITION ==================
    switch(t) {
...
Building(int id, Vec3 pos, int floors, int cluster, int zoneID = -1)
    ...
    weeklyAverage(0),
    pressureSimulated(NOMINAL_PRESSURE) { 
...
if (showSensorLabels) {
    ...
    else if (tcp_leak)    ss << " [LEAK]";
    else if (tcp_blockage) ss << " [BLOCKAGE]";
